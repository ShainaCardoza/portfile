document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleFormSubmit(e) {
        ...
        if (isEditing) {
            ...
            // SENDING MESSAGE (Backend) + SAVING CONTACT
            ...
            try {
                ...
                if (response.ok) {
                    alert('Message sent and stored in database successfully!');

                    // Save Contact Locally as well
                    addContact({ id: Date.now(), name, email, phone });

                    resetForm();
                } else {
                    alert('Oops! There was a problem submitting your form to the server.');
                }
            } catch (error) {
